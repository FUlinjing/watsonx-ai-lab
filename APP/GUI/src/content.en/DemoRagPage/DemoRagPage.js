...
import sourcePDF from "./AI ACT EN.pdf";
import {
  Breadcrumb,
  BreadcrumbItem,
  Button,
...
/* temporary removing WA interface
...
const DemoRagPage = () => {
  ...
  let [errorMessage, setErrorMessage] = useState("Processing error");
      ...
      .then(result =>
        {
          ...
          console.log("DEBUG Prompt used: ", result.prompt)
          if (!resp.ok)
          {
            setIsError(true)
            setIsLoading(false)
            setErrorMessage("No question")
            raiseNotification()
          }
          else if (result['answer']==='')
          {
            setIsError(true)
            setIsLoading(false)
            setErrorMessage("Server response time has elapsed")
            raiseNotification()
          }
          else
          {
  ...
  return <Grid className="landing-page" fullWidth>
    <Column lg={16} md={8} sm={4} className="landing-page__banner">
      ...
      <h1 className="landing-page__heading">Discover the new generation</h1>
          ...
          <TabPanel className={[isAnswered && 'background_change']} >
            <Grid className="tabs-group-content" >
              <Column md={4} lg={8} sm={4} className="landing-page__tab-content">
                {isAnswered===false && <div className='question_box'>
                  ...
                  <Section level={3}><Heading>Questions to the documentation</Heading></Section>
                  <div className="container_question">
                    <Search className="question_input" size="lg" placeholder="Write your question..." labelText="Search" closeButtonLabelText="" id="text-input" onChange={handleText} />
                    <Button className="question_button" onClick={sendRequest}>Search</Button>
                    {isLoading && <InlineLoading status="active" iconDescription="Loading" description="Processing..."/>}
            ...
            {isAnswered && <div>
              <div className='above_table'>
                ...
                <Button className='bottomContainer' kind="ghost" onClick={nextQuestion}>Ask another question</Button> 
              </div>
              <div className='answer_area'>
                <div><h4 className='answer_text'><CheckmarkOutline style={{ color: "green" }}/> {resAnswer}</h4></div>
              </div>
              <div className='above_table'>
                <h5 className='answer_area'>Where to find more information on this topic?</h5> 
              </div>
          ...
          <TabPanel className='background_change'>
            <Grid className="tabs-group-content">
              <Column lg={16} md={8} sm={4} className="landing-page__tab-content">
                Instruction {env.REACT_APP_WA_REGION}
              </Column>
            </Grid>
          </TabPanel>
        </TabPanels>
